...
#training the ANN
chunk_number = 0
for training_set, dataset in zip(pd.read_csv('train_preprocessed.csv', chunksize = 10000),
                                 pd.read_csv('train_no_header.csv', chunksize = 10000)):
    print('Chunk number: ' & str(chunk_number) & ' Number of chunks ~150')
    X_train = training_set.iloc[:, :].values
